...
export function DFTVisualizer() {
  ...
  const [useHannWindow, setUseHannWindow] = useState(false);
  ...
  const {
    ...
    twiddleFactors,
    currentSample,
  } = useDFTCalculation(analyserNode, sampleWindow, selectedFrequencyBin, isPlaying, useHannWindow);
      ...
      <main className="flex flex-col gap-4 flex-1 overflow-y-auto pb-4 px-4">
        ...
        <div className="h-96 flex-shrink-0">
          <SpectrumAnalyzer
            ...
            onHannWindowChange={setUseHannWindow}
          />
        </div>
      </main>
    </div>
